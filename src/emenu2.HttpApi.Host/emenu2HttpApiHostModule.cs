...
public class emenu2HttpApiHostModule : AbpModule
{
    ...
    private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<emenu2DomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}emenu2.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<emenu2DomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}emenu2.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<emenu2ApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}emenu2.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<emenu2ApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}emenu2.Application"));
            });
        }
    }
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(emenu2ApplicationModule).Assembly);
        });
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "emenu2 API");

            var configuration = context.ServiceProvider.GetRequiredService<IConfiguration>();
            c.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
            c.OAuthScopes("emenu2");
        });
